...
class DrugSearchService
{
    ...
    private function generateLetterVariations(string $term, array $similarLetters): array
    {
        ...
        // برای هر موقعیت در کلمه
        for ($i = 0; $i < $length; $i++) {
            $char = mb_substr($term, $i, 1);
            
            // برای هر گروه از حروف مشابه
        ...
        // همچنین ترکیبات چند حرفی را هم بررسی کنیم
        for ($i = 0; $i < $length - 1; $i++) {
            $twoChars = mb_substr($term, $i, 2);
            
            // برخی ترکیبات خاص مثل "خا" و "حا"
            if ($twoChars === 'خا') {
                $variations[] = mb_substr($term, 0, $i) . 'حا' . mb_substr($term, $i + 2);
            } elseif ($twoChars === 'حا') {
                $variations[] = mb_substr($term, 0, $i) . 'خا' . mb_substr($term, $i + 2);
            }
        }
    ...
    private function addPossibleSpaces(string $word): string
    {
        ...
        $commonPrefixes = ['سر', 'گوش', 'دست', 'پا', 'چشم', 'دل', 'معده', 'کبد', 'قلب', 'مغز'];
        $commonSuffixes = ['درد', 'خور', 'خوراکی', 'خون', 'آور', 'سوز', 'گیر', 'کش'];
        ...
        foreach ($commonPrefixes as $prefix) {
            if (mb_strpos($word, $prefix) === 0 && mb_strlen($word) > mb_strlen($prefix)) {
                ...
                // بررسی کن که بقیه کلمه معنی‌دار باشد
                if (mb_strlen($rest) >= 2) {
                    return $prefix . ' ' . $rest;
                }
            }
        ...
        foreach ($commonSuffixes as $suffix) {
            if (mb_substr($word, -mb_strlen($suffix)) === $suffix && mb_strlen($word) > mb_strlen($suffix)) {
    ...
    {
        // برای PostgreSQL از عملگر ~ استفاده می‌کنیم
        $query->orWhere(function ($q) use ($term, $fields) {
            // ایجاد الگوی regex برای PostgreSQL
            $pattern = $this->createPostgresRegexPattern($term);
            
            foreach ($fields as $field) {
                // استفاده از ~ برای PostgreSQL به جای REGEXP
                $q->orWhereRaw("CAST({$field} AS TEXT) ~ ?", [$pattern]);
            }
        });
    }
     ...
     * ایجاد الگوی regex برای PostgreSQL
     */
    private function createPostgresRegexPattern(string $term): string
    {
        ...
        for ($i = 0; $i < mb_strlen($term); $i++) {
            $char = mb_substr($term, $i, 1);
            $group = $this->findLetterGroup($char, $letters);
            
            // برای هر حرف، گروه مشابه‌ها را به صورت [abc] در می‌آوریم
            if (count($group) > 1) {
                $pattern .= '[' . implode('', $group) . ']';
            } else {
                $pattern .= $char;
            }
        }
    ...
    /**
     * جستجوی بهبود یافته با استفاده از Levenshtein distance (برای PostgreSQL)
     */
    public function fuzzySearchWithLevenshtein(string $term, int $threshold = 3)
    {
        // این متد نیاز به extension pg_trgm در PostgreSQL دارد
        return DrugInfo::where(function ($query) use ($term, $threshold) {
            // استفاده از similarity برای جستجوی fuzzy
            $query->whereRaw("similarity(nam_fa, ?) > ?", [$term, 0.3])
                  ->orWhereRaw("similarity(nam_en, ?) > ?", [$term, 0.3])
                  ->orWhereRaw("similarity(mavaredmasraf, ?) > ?", [$term, 0.3]);
        })
        ->orderByRaw("similarity(nam_fa, '{$term}') DESC")
        ->get();
    }
